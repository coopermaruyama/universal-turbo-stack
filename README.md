...
> **Note**
> The [db](./packages/db) package is preconfigured to use serverless postgres and is **edge-bound** with the [Neon Serverless](https://github.com/neondatabase/serverless) driver in production, and regular postgres in dev. If you're using something else, make the necessary modifications to the [schema](./packages/db/src/schema.ts) as well as the [client](./packages/db/src/index.ts) and the [drizzle config](./packages/db/drizzle.config.ts). If you want to switch to non-edge database driver, remove `export const runtime = "edge";` from all pages and api routes
